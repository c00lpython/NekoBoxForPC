//! Парсер Incy конфигураций на Rust.

use crate::models::{ProfileConfig, ProtocolType};
use crate::parser::{ConfigParser, ParseResult};

pub struct IncyParser;

impl ConfigParser for IncyParser {
    ...
    fn parse(&self, content: &str) -> ParseResult {
        ...
        if text.starts_with('{') && text.ends_with('}') {
            if let Ok(json_val) = serde_json::from_str::<serde_json::Value>(text) {
                ...
                if let Some(nodes) = json_val.get("nodes").and_then(|n| n.as_array()) {
                    for item in nodes {
                        let name = item.get("name").and_then(|n| n.as_str()).unwrap_or("Incy Node");
                        let host = item.get("host").and_then(|h| h.as_str()).unwrap_or("127.0.0.1");
        ...
        for line in text.lines() {
            ...
            if l.starts_with("incy://") {
                if let Ok(url) = url::Url::parse(l) {
                    ...
                    let name = url.fragment().unwrap_or("Incy Proxy");
